...
/*:
 - Callout(Apple documentation):
  ...
  Ok so let's go, our data streams are exposed by `Publishers` and to interact with these streams we have to use `Subscribers`. We can play with it and we will figure out how it works along the way.
  */
 import Combine
/*:
  How do I create a publisher 🤔 ?
  Ok maybe we need a little more context.
 ...
 this means a `Subscriber` can only subscribe to a `Publiser` if it reads the type of data the `Publisher` sends. It makes sense since we love type safety.
 ...
 - *3* is clear enough, we do something when the streams complete or returns an error.
  - *2* is the same when we receive a value, but I do not understand what `Subscribers.Demand` is and how it is used
  - *1* No clue of what a subscription is and what we are supposed to do with it.

 ## 3. Subscription

 `Subscription` less documented because we do not need to know more about it to make most things in combines so we are going to keep it very short here hand we might come back to it later.

 In [introduction to combine](https://developer.apple.com/videos/play/wwdc2019/722/) Tony Parker mentions it only two times and descibes it like this :

 A subscription is how a Subscriber controls the flow of data from a Publisher to a Subscriber.

 A `Demand` from the subscriber enable for it to control the amount of data that is sent to it, but more oftent than not the subscriber request for an unlimited amount of data. For that reason it is not higly important to know in depth how subscriber and publisher communicate together.

 ## 4. Life cycle

 This diageam from the wwdc give a good understanding of how the life cycle of a publisher works.
 ...
 This is all you need to now for a basic use of combine.
 */
//: [Next](@next)
