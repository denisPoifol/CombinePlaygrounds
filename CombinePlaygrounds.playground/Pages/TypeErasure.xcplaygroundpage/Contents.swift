...
/*:
 ...
 We have seen that the publisher modifiers proposed by combined relies heavily on generic implementation of publisher which can make us end up manipulating monstruously long types.
 ...
 Let's say I want to define a protocol that expose a publisher, I need a type for this publisher, here is what I can do:
 ...
 - **use an associated type for my protocol** but my protocol now cannot have an existencial type (ex `let myVar: ProtocolName`)
...
/*:
 This coding style enables us to keep a declarative approach and it is way easier to read, so even if it is for a single use somewhere in your codebase you should always favor an extension and make it private if you do not want it to be used elsewhere.

 >Type erasure comes at a cost, we saw earlier the developpers of the framework thought `map(_:)` on a `Publishers.Sequence<Array<Int>, Never>` should return a value of the same type instead of `Publishers.Map<Publishers.Sequence<Array<Int>, Never>, Int>`(probably for performance reasons) which means `eraseToAnyPublisher()` should not be abused. And like most things in programming type erasing is a trade of, it should be thought over.
