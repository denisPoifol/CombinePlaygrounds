...
let subscriber = Subscribers.Sink<Int, Never>(
    receiveCompletion: { completion in
        print("received a completion : \(completion)", to: &Logger.shared)
    },
    receiveValue: { value in
        print("received a value : \(value)", to: &Logger.shared)
    }
)
...
justPublisher.sink(
    receiveCompletion: {
        print("received a completion : \($0)", to: &Logger.shared)
        justPublisher.sinkPrint()
    }
) { print("received a value : \($0)", to: &Logger.shared) }
Logger.shared.returnLogs()
/*:
